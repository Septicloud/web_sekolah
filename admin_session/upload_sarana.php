...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!isset($_FILES['file_foto']) || $_FILES['file_foto']['error'] !== UPLOAD_ERR_OK) {
        header("Location: sarana_prasarana.php?error=upload_failed");
        exit;
    }
    ...
    if (move_uploaded_file($file_tmp, $target_file)) {
        ...
        // Simpan ke database
        $stmt = mysqli_prepare($koneksi, "INSERT INTO sarana_prasarana (nama_sarana, deskripsi, file_foto) VALUES (?, ?, ?)");
        mysqli_stmt_bind_param($stmt, "sss", $nama_sarana, $deskripsi, $new_file_name);
        
        if (mysqli_stmt_execute($stmt)) {
            mysqli_stmt_close($stmt);
            header("Location: sarana_prasarana.php?success=added");
            exit;
        } else {
            mysqli_stmt_close($stmt);
